...
/********PASO AL PROC.2**********/
        for(j=(int)nPEnt/2; j<(int)nPEnt; j++)
        {
            ...
            {
                fscanf(fentr,"%f",&dato);
                patrones[ ((j-(nPEnt/2))%50) * ( dim + 1 ) + i ] 
= dato;		
            }
            if( ((j-(nPEnt/2))%50) == 49 )
            {
                EscribirBloqueFlotantes( BASE, 50 * (dim+1), 
patrones );
                PonerValorSemaforo( 1, VERDE );
                while( ObtenerValorSemaforo( 1 ) == VERDE );
            }
...
/***** Lectura de los patrones de ENTRENAMIENTO      ******/
        for( j=0 ; j< nPE ; j++ )
        {
            ...
            for( i=0 ; i<(int)(dim+1) ; i++ )
                    memglobal[ j * (dim+1) + i ] = 
(float)bufflotantes[ ((j%50)*(int)(dim+1)) 
+i ];
...
/***** Lectura de los patrones de ENTRENAMIENTO      ******/
        for( j=k/2 ; j<k; j++ ) {
            while( (*semaforo2) == ROJO );
               for( i=0 ; i<(dim+1) ; i++ )
                   memglobal[ ( (j-k/2)  * (int)(dim+1) ) + i ] 
= (float)bufflotantes[ (((j-k/2)%50)*(int)(dim+1))+i ];
...
(int) resul[0], resul[1], (int) resul[2]);
        }
        i+=(int)Ventana[1];
    }
