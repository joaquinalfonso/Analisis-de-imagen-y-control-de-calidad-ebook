...
void main(int argc, char *argv[])
{
    ...
    if( argc != 4 ) {
        ...
        fprintf(stderr,"USO:\t pcicmeuc  <imagen.bmp>   <parametros.cm> <patrones.ent>\n");
        exit(ERROR);
    }
    ...
    /*********** Inicializar DSP's **************/
    InicializarPCIC44( "cm.out", "eucl.out" );
    ...
    PonerValorSemaforo( 2, VERDE );         /* SemHOST */
    PonerValorSemaforo( 3, VERDE );         /* SemDSP1 */

    PonerValorSemaforo( 7, ROJO );          /* SemRESULT */
    ...
    numRes = (altoBMP / TamVY) * (anchoBMP / TamVX);
    for( j=0 ; j<numRes ; j++ ) {
