...
unsigned VecinoMasCercano(float *vector, float *distancia);
void CMyClasif(void);
...
unsigned VecinoMasCercano(float *vector, float *distancia)
{
    ...
    dMin = 1000000000.0;        /* INFINITO inicialmente */
    pMin = nPEnt + 1;
    for(j=0;j<nPEnt;j++) {
        dist = 0.0;
        for(i=0;i<dim;i++)
            dist += (pEntr[j][i] - vector[i]) * (pEntr[j][i] - vector[i]);
...
*/
        ...
        vec = VecinoMasCercano(CM,&dist);
        printf("%03ld Vec: %03d  Dist: %f  Etiq: %d\n", contventana, (int) vec, dist, (int)pEntr[vec][dim]);
